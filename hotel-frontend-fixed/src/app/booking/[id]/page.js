...
// Define API_URL once at the top
const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000";

// --- UI Helper: Updated with reliable high-res URLs ---
...
export default function BookingPage() {
  ...
  // LOGIC: Load room from RENDER BACKEND
  useEffect(() => {
    if (!id) return;
    const loadRoom = async () => {
      try {
        ...
        const price = roomData.base_price || 0;
  ...
  // LOGIC: CREATE BOOKING on RENDER BACKEND
                ...
                <div className="bg-white p-6 rounded-2xl shadow-xl border border-blue-100 sticky top-24">
                    ...
                    <div className="flex justify-between items-center mb-6 text-sm">
                        <div>
                            ...
                            <p className="font-semibold text-slate-800 bg-gray-50 px-3 py-2 rounded-lg border border-gray-200">{check_in}</p>
                        </div>
                        <div className="text-gray-400 mt-4">➝</div>
                        <div className="text-right">
                            <p className="text-gray-500 mb-1">Check-out</p>
                            <p className="font-semibold text-slate-800 bg-gray-50 px-3 py-2 rounded-lg border border-gray-200">{check_out}</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3 pt-2">
                        ...
                        <div className="border-t pt-4 mt-4 flex justify-between items-center">
                            ...
                            <span className="font-extrabold text-2xl text-blue-600">PKR {totalPrice}</span>
                        </div>
                    </div>
                    ...
                    <p className="text-xs text-center text-gray-400 mt-4">Secure payment processing on the next step.</p>
                </div>
            </div>
        </div>
      </div>
